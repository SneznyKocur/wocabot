...
class wocabee:
    def __init__(self, udaje: tuple):
        ...
        self.driver = webdriver.Firefox(options=options) # type: ignore
    ...
    def wait_for_element_in_element(self, timeout, elem, by, element):
        WebDriverWait(self.driver, timeout).until(
            lambda x: elem.find_element(by, element).is_displayed()
        )
        return self.get_element(by, element)
    ...
    def get_classes(self) -> list:
        ...
        classesList = [
            ...
            for i, button in enumerate(
                classes.find_elements(By.CLASS_NAME, "btn-wocagrey")
            )
        ]
        return classesList
    ...
    def get_leaderboard(self):
        ...
        for student in students:
            ...
            online = "status-online" in student.find_element(
                By.CLASS_NAME, "status-icon"
            ).get_attribute("class")
        ...
        if prac == self.GETPACKAGE:
            ...
            def process_package(elem):
                try:
                    ...
                    playable = self.exists_element(
                        elem, By.CLASS_NAME, "fa-play-circle"
                    )
    ...
    def get_points(self, target_wocapoints=None):
        ...
        print(
            f"{self.info} Doing {difference} wocapoints ({wocapoints} -> {target_wocapoints})"
        )
    ...
    def learn(self, echo=False):
        while self.exists_element(self.driver, By.ID, "intro"):
            ...
            if self.exists_element(self.driver, By.ID, "pictureThumbnail"):
                ...
                self.dictionary_put(
                    os.path.basename(path)[:-4], translation, Picture=True
                )
    ...
    def _choose_picture(self):
        while True:
            ...
            picture = self.get_element(By.CLASS_NAME, "slick-current").find_element(
                By.TAG_NAME, "img"
            )
    ...
    def _complete_word(self):
        ...
        self.get_element(By.ID, "completeWordAnswer").send_keys(
            "".join(self.find_missing_letters(miss, preklad))
        )
        try:
        ...
        for i, x in enumerate(words):
            ...
            if x != sortable_text[i]:
                ...
                ActionChains(self.driver).drag_and_drop(
                    sortable[sortable_text.index(x)], sortable[i]
                ).perform()
        ...
        if self.exists_element(self.driver, By.ID, "introRun"):
            ...
            while self.exists_element(self.driver, By.ID, "introNext"):
                ...
                if picture:
                    ...
                    path = picture.get_attribute("src")
                    print(
                        f"{self.debug} PUT {os.path.basename(path)[:-4]} as translation"
                    )
                    self.dictionary_put(
                        os.path.basename(path)[:-4], translation.text, Picture=True
                    )
                try:
                    self.wait_for_element(10, By.ID, "introNext")
            ...
            print(
                f"{self.warn} Package has been already started before, words may not be in dictionary!"
            )
